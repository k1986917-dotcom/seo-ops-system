...
class SEOQualityRater:
    ...
    # Intent detection signals (融合 afa-seo dynamic intent scoring)
    COMMERCIAL_SIGNALS = ['best', 'review', 'vs', 'versus',
                          'comparison', 'compare', 'top',
                          'alternative', 'alternatives', 'buy',
                          'price', 'discount', 'cheap', 'budget']
    INFORMATIONAL_SIGNALS = ['how to', 'how do', 'what is',
                             'what are', 'why does', 'why is',
                             'guide', 'tutorial', 'tips',
                             'explained', 'learn', 'beginner',
    ...
    # 实体共现词族字典 — 基于实体 SEO (Entity-Based SEO) 的语义关联词校验
    # 覆盖常见激光笔话题的语义关联实体词
    ENTITY_COOCCURRENCE = {
        "burning": ["thermal", "focus", "collimation", "divergence", "heat", "absorb", "duty cycle", "diode"],
        "astronomy": ["stargazing", "visibility", "sky", "green", "beam", "constellation", "night", "star"],
        "safety": ["eyewear", "goggles", "retina", "blink", "reflection", "class 4", "class 3", "protection"],
        "blue laser": ["450nm", "nichia", "multimode", "thermal", "heat sink", "copper", "stainless"],
        "green laser": ["520nm", "532nm", "dpss", "direct diode", "infrared", "crystal", "brightness"],
        "buying guide": ["price", "budget", "warranty", "battery", "wavelength", "power", "specs", "package"],
        "power": ["mw", "watt", "output", "driver", "battery", "runtime", "discharge", "capacity"],
        "focusing": ["lens", "collimation", "divergence", "beam", "adjustable", "thread", "mechanism"],
    }

    # 共现覆盖率最低要求（低于此值警告）
    MIN_COOCCURRENCE_RATIO = 0.50

    def _detect_intent(self, content: str) -> str:
        """
        Detect article intent from H1 and opening content.
        Returns 'commercial', 'informational', or 'unknown'.
        (融合 afa-seo dynamic intent scoring)
        """
        h1_match = re.search(r'^#\s+(.+)$', content, re.MULTILINE)
        h1_text = h1_match.group(1).lower() if h1_match else ''
        first_section = content[:500].lower()
        combined = h1_text + ' ' + first_section

        commercial_hits = sum(
            1 for signal in self.COMMERCIAL_SIGNALS
            if signal in combined
        )
        informational_hits = sum(
            1 for signal in self.INFORMATIONAL_SIGNALS
            if signal in combined
        )

        if commercial_hits > informational_hits:
            return 'commercial'
        elif informational_hits > commercial_hits:
            return 'informational'
        elif commercial_hits > 0:
            return 'commercial'
        return 'unknown'
    ...
    def rate(
        ...
        custom_entity_matrix: Optional[Dict[str, List[str]]] = None
    ) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            site_domain: Site domain for internal link detection (e.g. 'laserpointerhub.com')
            custom_entity_matrix: Dynamic entity co-occurrence terms from SERP (overrides hardcoded defaults)

        Returns:
            Dict with overall score, category scores, and recommendations
        """
        # Detect article intent (dynamic intent scoring)
        ...
        # Dynamic intent-based scoring adjustments (融合 afa-seo)

        if intent == 'commercial':
            # Commercial articles (best/review/vs) MUST have a comparison table
            has_table = bool(re.search(
                r'\|.+\|.+\|\s*\n\s*\|[-:]+\|', content
            ))
            if not has_table:
                structure_score['score'] = max(0, structure_score['score'] - 20)
                structure_score['critical'] = structure_score.get('critical', [])
                structure_score['critical'].append(
                    '商业调研型文章（best/review/vs）必须包含对比表格 (Comparison Table) '
                    '— 当前未检测到 Markdown 表格结构。请在文章中添加产品/方案对比表格，'
                    '格式：| 产品 | 优点 | 缺点 | 价格 | ... |'
                )

        elif intent == 'informational':
            # Informational articles benefit from FAQ section
            has_faq = bool(re.search(
                r'(?:##+\s+.*(?:FAQ|Frequently Asked|Common Questions))',
                content, re.IGNORECASE
            ))
            if not has_faq:
                readability_score['suggestions'] = readability_score.get('suggestions', [])
                readability_score['suggestions'].append(
                    '信息型文章建议包含 FAQ 区块 (Frequently Asked Questions)，'
                    '提升 E-E-A-T 并增加 Featured Snippet 机会。建议在文章末尾添加 ## FAQ 区块。'
                )
        ...
        # Entity co-occurrence check (实体共现词校验)
        # 优先使用动态传入的 custom_entity_matrix (从 SERP 前5自动提取)
        entity_result = None
        if primary_keyword:
            entity_result = self._check_entity_cooccurrence(content, primary_keyword, custom_entity_matrix)
        ...
        if entity_result and entity_result['matched_topic']:
            if entity_result['coverage'] < self.MIN_COOCCURRENCE_RATIO:
                suggestions.append(
                    f"语义实体共现不足 ({entity_result['matched_topic']}): "
                    f"覆盖率 {entity_result['coverage']:.0%} (需≥{self.MIN_COOCCURRENCE_RATIO:.0%})。"
                    f"缺少词: {', '.join(entity_result['missing'][:5])}"
                )
    ...
    def _check_entity_cooccurrence(self, content: str, primary_keyword: str, custom_entities: Optional[Dict[str, List[str]]] = None) -> Dict[str, Any]:
        """
        实体共现词校验 — 取代传统关键词密度堆砌检测。
        基于实体 SEO (Entity-Based SEO)，校验文章是否自然覆盖了
        与主题相关的语义关联实体词（co-occurrence entities）。

        优先级: custom_entities (动态从 SERP 提取) > ENTITY_COOCCURRENCE (硬编码默认)
        """
        content_lower = content.lower()

        # 优先使用动态传入的实体词（从 SERP 前5提取）
        entity_source = custom_entities or self.ENTITY_COOCCURRENCE

        # 匹配最相关的词族
        related_entities = []
        matched_topic = None
        for topic, entities in entity_source.items():
            if topic in primary_keyword.lower():
                related_entities = entities
                matched_topic = topic
                break

        if not related_entities:
            return {'score': 100, 'matched_topic': None, 'found': [], 'missing': [], 'coverage': 1.0}

        found = []
        missing = []
        for entity in related_entities:
            if re.search(r'\b' + re.escape(entity) + r'\w*\b', content_lower, re.IGNORECASE):
                found.append(entity)
            else:
                missing.append(entity)

        coverage = len(found) / len(related_entities) if related_entities else 1.0
        score = 100 if coverage >= self.MIN_COOCCURRENCE_RATIO else max(0, round(coverage * 200))
    ...
    def _score_links(
        ...
        if internal_count is None and site_domain:
            internal_pattern = rf'\[([^\]]+)\]\(https?://(?:www\.)?{re.escape(site_domain)}'
            internal_count = len(re.findall(internal_pattern, content))
        elif internal_count is None:
            internal_count = 0

        if external_count is None:
            all_links = len(re.findall(r'\[([^\]]+)\]\(https?://', content))
            external_count = all_links - (internal_count if internal_count is not None else 0)
