...
class ContentScrubber:
    ...
    # AI fluff patterns — zero-information-density phrases to strip entirely
    # (融合 afa-seo anti-patterns: 无信息密度空话拦截)
    AI_FLUFF_PATTERNS = [
        # "As we all know, X is very/extremely important because..." → strip
        (r"[Aa]s we all know,?\s?[\w\s]+ is (?:very|extremely|incredibly|particularly) important (?:because|in order to|for)\s?", ""),
        # "It is worth noting that X plays an indispensable role in..."
        (r"[Ii]t is worth noting that [\w\s]+ plays an indispensable (?:role|part) in\s?", ""),
        # "In this challenging world/era/landscape..."
        (r"[Ii]n this (?:challenging|fast-paced|ever-changing|rapidly evolving) (?:world|era|landscape|age|environment),?\s?", ""),
        # "Needless to say, ..." → strip
        (r"[Nn]eedless to say,?\s?", ""),
        # "It goes without saying that..."
        (r"[Ii]t goes without saying that\s?", ""),
        # "That being said, ..." → strip
        (r"[Tt]hat being said,?\s?", ""),
        # "In other words, ..." → strip (AI over-explaining pattern)
        (r"[Ii]n other words,?\s?", ""),
        # "Due to the fact that..." → "Because"
        (r"[Dd]ue to the fact that\s?", "because "),
        # "As a matter of fact, ..." → strip
        (r"[Aa]s a matter of fact,?\s?", ""),
        # "It should be noted that..."
        (r"[Ii]t should be noted that\s?", ""),
        # "First and foremost, ..." → strip (redundant with "First,")
        (r"[Ff]irst and foremost,?\s?", ""),
        # "Last but not least, ..." → strip
        (r"[Ll]ast but not least,?\s?", ""),
    ]
    ...
    def scrub(self, content: str) -> Tuple[str, Dict]:
        ...
        # Step 5: Remove AI fluff (zero-information-density filler sentences)
        content = self._remove_ai_fluff(content)
    ...
    def _replace_emdashes(self, content: str) -> str:
        ...
        """
        # Protect markdown link URLs from em-dash replacement
        url_placeholders = {}
        url_count = 0
        def _protect_url(m):
            nonlocal url_count
            key = f'__URL{url_count}__'
            url_placeholders[key] = m.group(0)
            url_count += 1
            return key
        content = re.sub(r'\]\([^)]+\)', _protect_url, content)
    ...
    def _remove_ai_fluff(self, content: str) -> str:
        """
        Remove zero-information-density AI filler sentences.
        (融合 afa-seo anti-patterns: 提高文章信息密度)

        Unlike _replace_ai_phrases which targets single words/phrases,
        this targets entire sentence fragments that add no value.
        """
        for pattern, replacement in self.AI_FLUFF_PATTERNS:
            content, count = re.subn(pattern, replacement, content, flags=re.MULTILINE)
            self.stats['ai_fluff_removed'] += count
        return content
...
def scrub_content(content: str, verbose: bool = False) -> str:
    ...
    if verbose:
        ...
        print(f"  - AI fluff passages removed: {stats['ai_fluff_removed']}")
