...
1. 已实现网页 W1b 单次点击最多两轮 AI 修订 + 复检；W2 也以批次调用，且
   每次 W2 修订后仍先经过 W1b。W1b/W2 计数分离，失败摘要和草稿 SHA 会保存
   给后续 AI 修订使用；现有 evidence/claim ledger 与硬 gate 未改动。
2. 下一段把同一批次/失败记忆能力通过 HTTP 接入 Hermes R3→W3 总控制器，
   并实现无搜索凭据时“使用已有资料继续 / 人工粘贴”的确认分支。
3. 网页红色“接受当前蚕食问题并继续”仅是原有人工确认的清晰文案/样式；仍只
   在两轮 W2 后、评分通过、检查器正常且仅蚕食阻塞时可用，不能放行事实或评分问题。
