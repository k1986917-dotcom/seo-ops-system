...
- 验证：Sectional 非 Web `240 passed`（排除唯一已知 MCP `/etc/mime.types` Web 路由
  ...
  `git diff --check` 全部通过。
- 已有完整终态候选仍在 active root（17 checkpoints/ledgers + 5 terminal artifacts）。
  本次尚未 push、尚未运行新 Shadow、尚未 promotion。下一次经审查的真实运行必须使用
  恰好一次 `--resume-existing --refresh-complete --ai-call-limit 36`，先原子归档旧终态；
  禁止手工删除 checkpoint 或终态文件。
