...
def test_candidate_selection_final_repair_rebuilds_all_gates_after_unrelated_drift(tmp_path):
    sections, shadow = _setup()
    calls = []
    target_attempts = 0
    ...
    select_contract = next(
        item
        for item in shadow["section_link_contracts"]["sections"]
        if item["section_id"] == output["section_id"]
    )
    assert (
        output["used_ids"]["article_links"] == select_contract["article_links"]["selected_ids"][:1]
    )
    assert (
        output["used_ids"]["product_links"] == select_contract["product_links"]["selected_ids"][:1]
    )


def test_candidate_selection_final_repair_ignores_malformed_first_repair(tmp_path):
    sections, shadow = _setup()
    target_attempts = 0

    def malformed_then_clean(system, user):
        nonlocal target_attempts
        package = _package_from_prompt(user)
        response = _response(package)
        if package["reader_stage"] != "select":
        ...
        if target_attempts == 1:
            return response.replace(product_id, "product-not-approved")
        if target_attempts == 2:
            return "malformed repair without response markers"
        return response
    ...
    result = run_section_generation_sequence(
        ...
        link_contracts=shadow["section_link_contracts"],
        context_manifest=shadow["context_manifest"],
        generate_text=malformed_then_clean,
    )
