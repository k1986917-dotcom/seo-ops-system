...
- 主题缺口/边界入口使用十二类开放来源查询，不把自动选中的分支词拼进每一路；全部未被 CMS 重复门拦截的种子可见，只选 5 个继续深挖。末端主意图聚类只合并输入 ID 的同页面职责换说法，不能新增主题或改资格。
...
- 新文章发现期只以同主意图/正文覆盖作为硬阻断；需求、来源强度、品牌适配和页面级材料分别显示为人工选择与制作准备度。资料发现不能冒充需求，重复主题归入最接近旧文，安全/法规/具体产品事实在制作前必须补可靠来源。
- 图谱/边界调研先把 SERP 预算分散到多个前沿，再逐条补 Tavily 来源；Firecrawl 对不同资料查询按结果名次轮询。每层 `input_refs` 都必须保留，不能把同一 SERP 下无关的正文拼给候选。
- GSC 调研仍优先为真实 PAA 补来源；24 小时复用不扣实际请求，因此成功 evidence 数可以高于本轮新请求预算，但不增加独立性。
