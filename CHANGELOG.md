...
- W0 写稿提示现在把 claim ledger 作为不可省略的最终交卷合同，同时置于系统提示和
  用户提示的末尾：必须输出“完整正文 → 单独一行 `===CLAIM_LEDGER===` → 合法 JSON”，
  不得使用代码围栏或在 JSON 后追加文字。该变化只提升模型格式遵循，不放松任何
  evidence/claim 校验或 W0/W1b/W2/W3 gate。
